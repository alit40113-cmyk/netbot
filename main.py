...
def start(message):
    ...
    if not check_sub(user_id):
        ...
        bot.send_message(user_id, f"⚠️ مرحباً بك في بوت {MY_RIGHTS}\nاشترك بالقناة أولاً:", reply_markup=markup)
        return
    # نظام التفعيل التلقائي يظل كما هو لضمان الأمان
...
def callback_handler(call):
    ...
    if call.data == "verify_sub": start(call.message)
        ...
        bot.send_message(uid, f"✅ تم تفعيلك بواسطة {MY_RIGHTS}!"); bot.edit_message_text(f"✅ تم تفعيل {uid}", ADMIN_ID, call.message.message_id)
        ...
        bot.edit_message_text("🛠️ اختر نوع الباقة (حماية فائق القوة):", chat_id, call.message.message_id, reply_markup=markup)
    elif call.data.startswith("mode_"): create_and_send_ultra_file(chat_id, call.data.split("_")[1])
...
def create_and_send_ultra_file(chat_id, mode):
    ...
    # --- التعديل هنا لإضافة حقوقك داخل الملف ---
    config = {
        "type": "VLESS",
        "name": f"🛡️ VVIP-{MY_RIGHTS}-{random.randint(100,999)}", # اسمك يظهر داخل التطبيق
        "isLocked": True,
        "vlessTunnelConfig": {
            "v2rayConfig": {
                "host": random_sni,
                ...
                "wsPath": f"/Live/Stream/{MY_RIGHTS}/{generate_strong_padding(5)}", # اسمك في المسار
                "wsHeaderHost": host
            },
            "injectConfig": {
        ...
        "owner": MY_RIGHTS # حقوق إضافية مخفية
    }
    
    encoded = base64.b64encode(json.dumps(config).encode()).decode()
    file_name = f"VVIP_{MY_RIGHTS}_{data['net']}.dark" # اسم الملف الخارجي باسمك
    with io.BytesIO(f"darktunnel://{encoded}".encode()) as dark_file:
        dark_file.name = file_name
        bot.send_document(chat_id, dark_file, caption=f"✅ **تم صنع الملف بواسطة {MY_RIGHTS}**\n📡 نوع الحماية: Ultra Stealth\n🔒 الملف مغلق ومشفر بالكامل.")
...
print(f"✅ Bot is Online by {MY_RIGHTS}")
